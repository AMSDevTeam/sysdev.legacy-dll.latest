...
#Region "Sub New"
        ...
        Sub New(ByVal server As String, ByVal database As String, ByVal userid As String, ByVal password As String, ByVal filename As String, ByVal tablecountexcluded As Integer)
            _server = server : _database = database : _userid = userid : _password = password
            _parameters.Clear() : _filename = filename : _error = String.Empty : If _maxallowedpacket <= 0 Then _maxallowedpacket = 16
            _TableCountExcluded = tablecountexcluded
        End Sub
#End Region
...
#Region "Properties"
        ...
        Dim _TableCountExcluded As Integer = 0
        ''' <summary>
        ''' Gets or sets Table Count Excluded
        ''' </summary>
        ''' <returns></returns>
        Public Property TableCountExcluded() As String
            Get
                Return _TableCountExcluded
            End Get
            Set(ByVal value As String)
                _TableCountExcluded = value
            End Set
        End Property
...
#Region "Methods"
        Private Sub ExtractResourceApplications()
            ...
            If Directory.Exists(curdir) Then
                If IO.File.Exists(curdir & "\mysql.exe") And
                ...
                If IO.File.Exists(curdir & "\mysqldump.exe") And
...
#Region "Internal Functions"
        ...
        Public Function Export() As Boolean
            ...
            Dim _getextension As String = System.IO.Path.GetExtension(_filename)
            Dim _tempbackup As String = Application.StartupPath & "\dbtemp." & _getextension.Replace(".", "")
            Dim _getpath As String = System.IO.Path.GetDirectoryName(_tempbackup)
            Dim _getfilename As String = "dbtemp"
            ...
            If CanExport Then
                ...
                '*** Delete Temporary Backup for rewriting the db dump file
                Try
                    If My.Computer.FileSystem.FileExists(_tempbackup) Then My.Computer.FileSystem.DeleteFile(_tempbackup, FileIO.UIOption.OnlyErrorDialogs, FileIO.RecycleOption.DeletePermanently, FileIO.UICancelOption.DoNothing)
                Catch ex As Exception
                End Try

                If _TableCountExcluded > 0 Then
                    For ictr As Integer = 1 To _TableCountExcluded
                        Try
                            Dim _2filename As String = _getfilename
                            _2filename = _getpath & "\" & _2filename & "_" & ictr & _getextension
                            If My.Computer.FileSystem.FileExists(_2filename) Then My.Computer.FileSystem.DeleteFile(_2filename, FileIO.UIOption.OnlyErrorDialogs, FileIO.RecycleOption.DeletePermanently, FileIO.UICancelOption.DoNothing)
                        Catch ex As Exception
                        End Try
                    Next
                End If
                ...
                Try
                    If My.Computer.FileSystem.FileExists(Application.StartupPath & "\dbmerge.bat") Then My.Computer.FileSystem.DeleteFile(Application.StartupPath & "\dbmerge.bat", FileIO.UIOption.OnlyErrorDialogs, FileIO.RecycleOption.DeletePermanently, FileIO.UICancelOption.DoNothing)
                Catch ex As Exception
                End Try
                ...
                For Each s As String In _parameters
                    If s.Trim.ToLower().Contains(" --where") = False Then
                        sParam &= " " & s.Trim
                    End If
                Next

                With sw
                    .WriteLine("""" & Application.StartupPath & "\mysqldump"" --host=" & _server & " --user=" & _userid & " --password=" & _password & " " & _database & " " & sParam & " --set-charset --default-character-set=utf8 >" & """" & _tempbackup & """")
                End With

                If _TableCountExcluded > 0 Then
                    sParam = String.Empty
                    Dim _ctr As Integer = 0
                    Dim _filename2 As String = _getfilename
                    For Each s As String In _parameters
                        Try
                            If s.Trim.ToLower().Contains(" --where") = True Then
                                _ctr += 1
                                sParam = s.Trim
                                _filename2 = _getpath & "\" & _getfilename & "_" & _ctr & _getextension
                                With sw
                                    .WriteLine("""" & Application.StartupPath & "\mysqldump"" --host=" & _server & " --user=" & _userid & " --password=" & _password & " " & _database & " " & sParam & " --skip-comments --set-charset " & " --quick --default-character-set=utf8 --extended-insert >" & """" & _filename2 & """")
                                End With
                            End If
                        Catch ex As Exception
                        End Try
                ...
                Dim _copyfile As String = "COPY """ & Application.StartupPath & "\dbtemp." & _getextension.Replace(".", "") & """ "
                If _TableCountExcluded > 0 Then
                    Try
                        Dim _filename2 As String = _getfilename
                        For iCtr As Integer = 1 To _TableCountExcluded
                            _filename2 = _getpath & "\" & _getfilename & "_" & iCtr & _getextension
                            _copyfile &= "+ """ & _filename2 & """ "
                        Next
                    Catch ex As Exception
                    End Try
                End If

                '***Merge Backup and Image 
                _copyfile &= """" & _filename & """ "
                Dim _MergeFile As IO.StreamWriter
                _MergeFile = IO.File.CreateText(Application.StartupPath & "/dbmerge.bat")
                With _MergeFile
                    .WriteLine(_copyfile)
                    .Close()
                    .Dispose()
                ...
                proc = New Process
                With proc
                    .StartInfo.FileName = Application.StartupPath & "\dbmerge.bat"
                    .StartInfo.CreateNoWindow = True : .StartInfo.WindowStyle = ProcessWindowStyle.Hidden
                    .StartInfo.RedirectStandardError = True : .StartInfo.UseShellExecute = False
                    .Start()
                ...
                '***Delete temporary file for images
                If _TableCountExcluded > 0 Then
                    Try
                        Dim _filename2 As String = _getfilename
                        For iCtr As Integer = 1 To _TableCountExcluded
                            _filename2 = _getpath & "\" & _getfilename & "_" & iCtr & _getextension
                            Try
                                If My.Computer.FileSystem.FileExists(_filename2) Then My.Computer.FileSystem.DeleteFile(_filename2, FileIO.UIOption.OnlyErrorDialogs, FileIO.RecycleOption.DeletePermanently, FileIO.UICancelOption.DoNothing)
                            Catch ex As Exception
                            End Try
                        Next
                    Catch ex As Exception
        ...
        Private Function MergeByte(ByVal byteone As Byte(), ByVal bytetwo As Byte()) As Byte()

            Dim _byte() As Byte = New Byte((byteone.Length - 1) + (bytetwo.Length - 1)) {}
            Array.Copy(byteone, _byte, byteone.Length)
            Array.Copy(bytetwo, _byte, bytetwo.Length)
...
#Region "Shared Functions"
        ...
        Public Overloads Shared Function Backup(ByVal server As String, ByVal database As String, ByVal userid As String, ByVal password As String, ByVal filename As String, ByVal parameters As DumpParameterCollection, ByVal tableexcludedcount As Integer) As Boolean
            Dim d As New Dump(server, database, userid, password, filename, tableexcludedcount)
            If parameters.Count > 0 Then
                For Each s As String In parameters
                    d.Parameters.Add(s)
                Next
